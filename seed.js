  ...
  if (table) {
    db.serialize(function() {
      ...
      db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='Timesheet'", (error, table) => {
        ...
        if (table) {
          db.run(`INSERT INTO Timesheet (name, description, hours, rate, date, employee_id) VALUES ("seed name 1", "seed desc 1", 10, 15.5, 1506100907820, ${employeeId})`);
          db.run(`INSERT INTO Timesheet (name, description, hours, rate, date, employee_id) VALUES ("seed name 2", "seed desc 2", 8, 15.5, 1406100907820, ${employeeId})`);
        }
      });
    });
  }
